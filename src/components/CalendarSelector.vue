<template>
  <div class="flex flex-col justify-center items-center py-6 relative">
    <!-- Date Card -->
    <div
      class="bg-amber-100 h-40 w-32 sm:h-48 sm:w-40 md:h-48 md:w-44 rounded-2xl shadow-xl flex flex-col cursor-pointer"
      @click="toggleCalendar"
    >
      <div class="bg-green-950 h-1/3 rounded-t-2xl flex justify-center items-center shadow-inner">
        <span class="font-inter font-bold text-xl sm:text-2xl md:text-3xl text-white">{{
          day
        }}</span>
      </div>
      <div class="flex flex-col justify-center items-center flex-1">
        <span class="font-inter text-3xl sm:text-4xl md:text-5xl font-light text-green-950">{{
          month
        }}</span>
        <span class="font-inter text-4xl sm:text-6xl md:text-7xl font-bold text-green-950">{{
          date
        }}</span>
      </div>
    </div>
    ...
    <transition name="fade">
      <div
        v-if="showCalendar"
        class="absolute top-[100%] mt-0 bg-white rounded-xl shadow-lg border border-amber-200 p-4 z-10 w-64 sm:w-80 md:w-96"
      >
        <!-- Header -->
        <div class="flex justify-between items-center mb-4">
          <button
            @click="prevMonth"
            class="px-2 sm:px-3 py-1 bg-amber-100 rounded-lg hover:bg-amber-200 transition disabled:opacity-30 hover:cursor-pointer"
            :disabled="currentMonthIndex === 0"
          >
            ‹
          </button>
          <span class="font-inter font-bold text-lg sm:text-xl md:text-2xl text-green-950">
        ...
        <!-- Days of week -->
        <div
          class="grid grid-cols-7 gap-1 sm:gap-3 text-center font-sans font-light text-xs sm:text-sm md:text-base text-green-900 mb-2"
        >
          <span v-for="d in days" :key="d">{{ d }}</span>
        </div>

        <!-- Calendar dates -->
        <div class="grid grid-cols-7 gap-x-1 sm:gap-x-3 gap-y-1 text-center">
          <div
            v-for="(dayObj, i) in calendarDays"
            :key="i"
            :class="[
              'py-1 sm:py-2 rounded-lg font-inter transition-colors duration-300 text-sm sm:text-base md:text-lg',
